...
| Firecrawl Free | 1,000 credits/month; basic scrape 1/page; search 2 per up-to-10 results | Sixteen searches + fourteen basic HTML scrapes is about 46 credits if every slot is used. A typical run is lower: a few local accounts, then one document search per step and a PDF follow-up only when the first results are homepages. Avoid premium formats and broad crawls. [Pricing](https://www.firecrawl.dev/pricing), [search billing](https://docs.firecrawl.dev/features/search). |
